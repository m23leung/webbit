...
specified by `thread list`, we know this is our worker thread because `thread list` specifies where the thread is defined in the file (and its current
...
want to quit. If you want to quit without being prompted, enter `quit unconditionally` (abbreviated to `q!`).

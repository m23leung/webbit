...
MiniProfiler.templates["profilerTemplate"] = function anonymous(it) {
  ...
  if (MiniProfiler.showTotalSqlCount()) {
    out +=
      ' <span class="profiler-number"> ' +
      it.sql_count +
      ' <span class="profiler-unit">sql</span> </span> ';
  }
  out +=
  ...
  if (it.has_sql_timings) {
    out +=
      ...
      MiniProfiler.formatDuration(
        (it.duration_milliseconds_in_sql / it.duration_milliseconds) * 100
      ) +
  ...
  if (arr2) {
    ...
    while (i2 < l2) {
      ...
      out +=
        ...
        ' ms of total request time"> ' +
        MiniProfiler.formatDuration(
          (it.custom_timing_stats[value].duration / it.duration_milliseconds) *
            100
        ) +
    ...
    if (arr3) {
      ...
      while (i3 < l3) {
        ...
        out +=
          ...
          value.name +
          '</td> <td class="profiler-duration"> ';
        if (value.duration >= 0) {
          out +=
            ' <span class="profiler-unit"></span>' +
            MiniProfiler.formatDuration(value.duration) +
            " ";
        }
        out +=
  ...
  if (it.custom_fields && Object.keys(it.custom_fields).length > 0) {
    out +=
      ' <p class="custom-fields-title">Snapshot custom fields</p> <table class="profiler-timings"> <tbody> ';
  ...
  if (it.has_sql_timings) {
    out +=
      ' <div class="profiler-queries"> <table> <thead> <tr> <th style="text-align:right">step<br />time from start<br />query type<br />duration</th> <th style="text-align:left">call stack<br />query</th> </tr> </thead> <tbody> ';
...
MiniProfiler.templates["linksTemplate"] = function anonymous(it) {
  ...
  if (it.custom_link) {
    out +=
      ' <a href="' +
      it.custom_link +
      '" class="profiler-custom-link" target="_blank">' +
      it.custom_link_name +
      "</a> ";
  }
  out += " ";
...
MiniProfiler.templates["timingTemplate"] = function anonymous(it) {
  ...
  out +=
    ...
    MiniProfiler.formatDuration(
      it.timing.duration_without_children_milliseconds
    ) +
  ...
  if (it.timing.has_sql_timings) {
    ...
    if (
      it.timing.executed_readers > 0 ||
      it.timing.executed_scalars > 0 ||
      it.timing.executed_non_queries > 0
    ) {
      out +=
        "" +
    ...
    while (i1 < l1) {
      ...
      if (it.timing.custom_timings && it.timing.custom_timings[value]) {
        out +=
          ...
          ' invocations in this step (excludes children)"> ' +
          MiniProfiler.formatDuration(
            it.timing.custom_timing_stats[value].duration
          ) +
          " </td> ";
      } else {
        out += ' <td colspan="2"></td> ';
...
MiniProfiler.templates["sqlTimingTemplate"] = function anonymous(it) {
  ...
  if (it.s.execute_type == 3) {
    out +=
      "first result fetched: " + it.s.first_fetch_duration_milliseconds + "ms";
  }
  out +=
    '">' +

...
$(function () {
  ...
  $("#drag_html5, #drag_html5_scroll").on("dragend", function (ev) {
    $(this).after(
      '<div class="log">DragEnd with client position: ' +
        ev.clientX +
        "," +
        ev.clientY
    );
  ...
  $("#drop_html5, #drop_html5_scroll").on("dragover", function (ev) {
    $(this).after(
      '<div class="log">DragOver with client position: ' +
        ev.clientX +
        "," +
        ev.clientY
    );
  ...
  $("#drop_html5, #drop_html5_scroll").on("dragleave", function (ev) {
    $(this).after(
      '<div class="log">DragLeave with client position: ' +
        ev.clientX +
        "," +
        ev.clientY
    );
  ...
  $("#drop_html5, #drop_html5_scroll").on("drop", function (ev) {
    $(this).after(
      '<div class="log">Drop with client position: ' +
        ev.clientX +
        "," +
        ev.clientY
    );
  ...
  $("#drop_html5, #drop_html5_scroll").on("drop", function (ev) {
    ...
    if (oev.dataTransfer.items) {
      ...
      for (var i = 0; i < oev.dataTransfer.types.length; i++) {
        ...
        $(this).append(
          "HTML5 Dropped string: " + type + " " + oev.dataTransfer.getData(type)
        );
      }
    }
  });
  ...
  $("#with_change_event").change(function () {
    $("body").append(
      $('<p class="change_event_triggered"></p>').text(this.value)
    );
  });
  $("#with_change_event").on("input", function () {
    $("body").append(
      $('<p class="input_event_triggered"></p>').text(this.value)
    );
  });
  $("#checkbox_with_event").click(function () {
    $("body").append(
      '<p id="checkbox_event_triggered">Checkbox event triggered</p>'
    );
  });
  ...
  $("#reload-link").click(function () {
    setTimeout(function () {
      $("#reload-me").replaceWith(
        '<div id="reload-me"><em><a>has been reloaded</a></em></div>'
      );
    }, 250);
  });
  ...
  $("#click-test").on({
    click: function (e) {
      window.click_delay =
        (new Date().getTime() - window.mouse_down_time) / 1000.0;
      var desc = "";
